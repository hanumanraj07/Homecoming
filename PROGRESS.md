...
## Backend end-to-end verification (post-3C)

The backend has now been tested against a **real, live, MongoDB-wire-protocol
database** — not curl-against-an-unreachable-DB like every prior session, an
actual full round trip. This closes the biggest recurring caveat in this
file: every backend limitation note above ("no live MongoDB in this
sandbox") is now out of date for the backend specifically (the mobile app
itself is still unverified — see below).

**How**: this sandbox still has no route to MongoDB Atlas or a downloadable
`mongod` binary (`mongodb.org` and every container registry's blob storage —
Docker Hub, AWS public ECR — are blocked at the CDN layer by this
environment's network policy; running `dockerd` itself actually works here,
pulling images from it doesn't). What did work: **FerretDB** — an
open-source server that speaks the real MongoDB wire protocol on top of a
different storage engine. Built FerretDB v1.24.2 from source (`go install`
alone panics — its version string is normally generated by their Taskfile
build, so it needs a git clone plus a hand-written `build/version/version.txt`
before `go build`), running with its SQLite backend (`--handler sqlite`) via
the Go module proxy (`proxy.golang.org`, which is unproxied/allowed here) —
no Docker, no external binary download, no Atlas account needed. FerretDB v2
was tried first and rejected: it requires a PostgreSQL extension
(`documentdb_api`) that isn't just a SQL script, it's a compiled Postgres
extension with its own build process — v1's SQLite backend was the pragmatic
choice here, not v2's Postgres one.

**What was tested** — every route in the app, end to end, against this real
database:
- Register, login (including wrong password → 401, duplicate email → 409),
  `GET /me`
- Guardian create ×2, list (confirmed primary-first sort), update, delete,
  delete-again → 404
- Journey create (confirmed `checkInDeadline` = `expectedArrival` + exactly
  10 minutes), create with a guardian ID that isn't yours → 400, create with
  a past `expectedArrival` → 422, two location PATCHes (confirmed the path
  array actually accumulates), get-by-id, list, check-in, check-in-again →
  409, location PATCH after check-in → 409
- Media upload with a real file (not curl's own multipart handling standing
  in for the app's — this exercises the same `multer` + disk storage path
  the app hits), fetched back byte-for-byte identical, a disallowed file
  type rejected with 400
- Incident create (SOS, linked to the journey, with the uploaded photo's
  URL), get-by-id, list, create with an invalid `type` → 422
- **Cross-user data isolation**: registered a second user and confirmed they
  get a 404 (not a leak) on the first user's incident and an empty list from
  `/api/guardians` — every query really is scoped to `req.user._id`, not
  just in the code but in what the database actually returns
- A malformed ObjectId in a URL → clean 422 from the validator, never
  reaches Mongoose, never a 500

**Result: everything passed.** No new backend bugs found this pass — the
two bugs from the 3C sweep (the camera mode race, the multipart header
override) were both frontend/client-side and are already fixed. This is
strong evidence the backend is solid; it is not proof the *mobile app*
talking to it is, since the app itself still hasn't run on a device.

**Not part of this repo.** FerretDB, its SQLite data file, and the
PostgreSQL instance briefly started for the v2 attempt all lived outside the
project (`/tmp`, stopped and deleted after testing) — nothing about this
verification method is checked into the repo, and `server/.env` was
restored to its original `mongodb://localhost:27017/homecoming` default
afterward. If you want to reproduce this locally: `go install
github.com/FerretDB/FerretDB/cmd/ferretdb@v1.24.2` won't work standalone for
the reason above; cloning the repo and building with a manually-created
`version.txt` does. Or just point `MONGO_URI` at Atlas or a real local
